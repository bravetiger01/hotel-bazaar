...
// Create a test transporter to validate email domains (for MX check only, not for sending)
...
// Send order notification email to admin using SendGrid
const sendOrderNotification = async (orderDetails, customerInfo) => {
  try {
    ...
    const msg = {
      to: process.env.EMAIL, // Admin email (verified sender)
      from: process.env.EMAIL, // Must be a verified sender in SendGrid
  ...
  console.log('sendOrderNotification: sending to', process.env.EMAIL);
  console.log('sendOrderNotification: msg', JSON.stringify(msg, null, 2));
  await sgMail.send(msg);
  console.log('sendOrderNotification: sent successfully');
  return { success: true };
  } catch (error) {
    console.error('SendGrid order notification error:', error);
    return { success: false, error: error.message };
  }
};
